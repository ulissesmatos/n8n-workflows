 ...
 */
  ...
  // Helper to wrap a Simple-Icons CDN URL in a standard 24×24 <img>
  ...
  // ---- Brand / Platform icons via Simple Icons CDN ----
  ...
  // Resolve a BRANDS entry to { svg, color, label }
  function resolveBrand(key) {
    var b = BRANDS[key];
    if (!b) return null;
    return {
      svg: si(b.slug, b.color),
      color: b.color,
  ...
  function resolveIcon(key) {
    ...
    // Check brands via CDN
    var brand = resolveBrand(key);
  ...
  export { getNodeIcon, extractNodes, renderBadges };
